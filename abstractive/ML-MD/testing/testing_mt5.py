...
class Summarizer(pl.LightningModule):
    ...
    def _generative_step(self, batch):
        ...
        # Remove forced_bos_token_id for mT5
    )
    ...
    def on_test_epoch_end(self):
        ...
        for key, value in self.languages_map.items():
            ...
            self.languages_map[key]['domain'] = [d for d, t in zip(domains, tgt_langs) if value[0] == t]
        ...
        for key in self.languages_map:
            ...
            df_key = pd.DataFrame({
                ...
                'domain': [self.languages_map[key]['domain'][i] for i in range(l)],
        ...
        save_path = f'abstractive/ML-MD/predictions/{model_nm}_prediction/lang_wise_combined_{model_nm}.csv'
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
        df_to_write.to_csv(save_path, index=False)
        print(f" Saved detailed results to {save_path}")
        self.test_outputs = []
        # Print summary to terminal
        print("Overall ROUGE-F1 Scores")
        print("R-1 F:", round(overall_rouge["rouge-1"]["f"], 4))
        print("R-2 F:", round(overall_rouge["rouge-2"]["f"], 4))
        print("R-L F:", round(overall_rouge["rouge-l"]["f"], 4))

        return { "rouge-1_f": overall_rouge["rouge-1"]["f"],"rouge-2_f": overall_rouge["rouge-2"]["f"],"rouge-l_f": overall_rouge["rouge-l"]["f"],"num_samples": len(pred_texts),}
...
if __name__ == "__main__":
    ...
    parser.add_argument('--prediction_path', default='preds_mt5.txt', help='path to save prediction file')
    ...
    logger=WandbLogger(name='inference_' + 'salience' + '_' + 'combined' +  '_' + model_name, save_dir='./', project='multilingual evaluation', log_model=False)
    ...
    print("Test complete. Results:", results)
